...
test.describe("Backspace with hidden deletions", () => {
  ...
  test.describe("Backspace in a paragraph", () => {
    test("Succeeds to delete multiple characters with Backspace one after another, but fails to skip over the hidden deletion", async ({
      page,
    }) => {
      ...
      // it fails to skip over the hidden deletion
      ...
      expect(cursorInfo.from).toBeGreaterThanOrEqual(delPos);
      expect(cursorInfo.to).toBeGreaterThanOrEqual(delPos);
      expect(cursorInfo.from).toBeLessThanOrEqual(delPos + delSize);
      expect(cursorInfo.to).toBeLessThanOrEqual(delPos + delSize);
